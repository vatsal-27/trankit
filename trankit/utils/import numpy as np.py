...
def probability(theta, pi, m, s, k, n_classes, continuous_mask, qc, device):
	...
	m_new = torch.reshape(s,(len(s),1,len(s[0])))
	y = torch.tensor(list(range(len(pi))),device=device)
	eq = torch.eq(k.view(-1, 1), y).double().t()
	r = qc * eq.squeeze() + (1 - qc) * (1 - eq.squeeze())
	params = torch.exp(pi)
	temp = Beta(r*params,params*(1-r))
	p = torch.exp(temp.log_prob(s_new))*m_new*continuous_mask+(1-m_new)
	p_s = torch.prod(p,-1)
	# print(p_s.shape,p.shape,r.shape,y.shape,eq.shape,s.shape,pi.shape)
	return p_l_y * p_s
