...
class TPipeline:
    ...
    def _set_up_config(self, training_config):
        ...
        if self._gpu and torch.cuda.is_available():
            self._use_gpu = True
            master_config.device = torch.device('cuda:1')
        else:
            self._use_gpu = False
    ...
    def _train_posdep(self):
        ...
        for epoch in range(50):
            ...
            T_LOSS +=[int(sum(loss_list)/len(loss_list))]

            dev_score, pred_conllu_fpath = self._eval_posdep(data_set=self.dev_set, batch_num=self.dev_batch_num,
                                                             name='dev', epoch=epoch)
            
            # loss_list = []
            # for batch in DataLoader(self.dev_set, batch_size=self._config.batch_size,
            #                     shuffle=False, collate_fn=self.dev_set.collate_fn):
            #     batch_size = len(batch.word_num)
            #     word_reprs, cls_reprs = self._embedding_layers.get_tagger_inputs(batch)
            #     loss = self._tagger.forward_without_grad(batch, word_reprs, cls_reprs)
            #     loss_list += [loss]
            ...
            plt.savefig("val_plot2.png")

            plt.figure()
            plt.yscale("log")
            plt.plot(V_SCORE,label="Val Score")
            plt.xlabel("epoch")
            plt.ylabel("Score")
            plt.title("Val Score Vs Epoch")
            plt.savefig("val_score_plot2.png")
    ...
    def _eval_posdep(self, data_set, batch_num, name, epoch):
        ...
        for batch in DataLoader(data_set, batch_size=self._config.batch_size,
            ...
            deprel_seqs = [[self._config.itos[DEPREL][predicted_dep[1][i][j + 1][h]] for j, h in
                           ...
                           in
                           enumerate(head_seqs)]
            # # deprel_seqs = [[self._config.itos[DEPREL][predicted_dep[1][i][j]] for j, h in
            # #                 enumerate(hs)] for
            # #                i, hs
            # #                in
            # #                enumerate(head_seqs)]
            ...
            for bid in range(batch_size):
                for i in range(batch.word_num[bid]):
                    ...
                    # head
                    # print(bid,i,len(predicted_dep[1]),len(predicted_dep[1][0]))
                    data_set.conllu_doc[sentid][wordid][HEAD] = int(head_seqs[bid][i])
                    # deprel
